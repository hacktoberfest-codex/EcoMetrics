...
export const Navbar = () => {
    ...
    return (
        <navbar>
            <nav
                className="flex justify-between items-center py-4 md:py-8 px-6 mx-auto max-w-screen-xl md:px-12 lg:px-16 xl:px-24  ">
                <button
                    ...
                    onClick={toggleMenu}
                >
                    {isMenuOpen ? <FaTimes className="h-6 w-6 text-4xl inline-flex"/> :
                        <GiHamburgerMenu className="h-6 w-6 text-4xl inline-flex"/>}
                ...
                >
                    <ul className="flex flex-col md:flex-row items-center space-y-6 md:space-y-0 md:space-x-6 lg:md:-x-8">
                        <li className="text-lg md:text-base lg:text-lg font-medium group">
                            ...
                            <div
                                className={`h-0.5 bg-green-500 scale-x-0 group-hover:scale-x-100 transition-transform origin-left rounded-full duration-300 ease-out`}></div>
                        </li>
                        <li className="text-lg md:text-base lg:text-lg font-medium group">
                            <NavLink
                                to="/Prevention"
                        ...
                        <li className="text-lg md:text-base lg:text-lg font-medium group flex-end">
                            <button className="bg-green-500 py-2 px-3 rounded-2xl">
                                <NavLink to="/login"
                                         className="text-lg md:text-base lg:text-lg font-medium group text-white">
                                    Login
                                </NavLink>
                            </button>
                        </li>
    ...
    rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition duration-500 focus:outline-none"
            ...
            >
                <div
                    className="flex  p-2 w-full bg-green-100 border border-green-600 space-x-2 rounded-lg items-center">
                    <BiSearchAlt className="text-2xl text-green-600"/>
                    <form onSubmit={(e) => handelSubmit(e)}>
                        <input
                            className="w-full  bg-green-100  outline-none border-transparent focus:border-transparent focus:ring-0 rounded-lg text-sm sm:text-base placeholder:text-green-800"
                            type="text"
